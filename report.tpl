...
{{#include}}criterion.css{{/include}}
    ...
    <h1>The Cost of Haskell Concurrency</h1>
    
Benchmarks of some simple operations using Haskell's concurrency constructs.

<h4>write-read</h4>
The main thread writes () to a reference, then reads from it.

<h4>forkIO-join</h4>
The main thread calls forkIO, then blocks on a synchronisation construct. The new thread writes () to the lock, thus un-blocking the main thread.

<h4>send-join</h4>
The main thread sends () to another thread using on of several synchronisation mechanisms. It then blocks on an MVar. The other thread reads the (), then writes to the MVar, un-blocking the main thread.
